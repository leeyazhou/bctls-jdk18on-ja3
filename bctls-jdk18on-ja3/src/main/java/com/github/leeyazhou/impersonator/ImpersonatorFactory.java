...
//			provider.configure(userAgent);
	...
	/**
	 * 4 -> 3 // SETTINGS_MAX_CONCURRENT_STREAMS renumbered. 7 -> 4 //
	 * SETTINGS_INITIAL_WINDOW_SIZE renumbered.
	 */
...
//		byte[] lastGreaseData = needGrease ? TlsUtils.EMPTY_BYTES : null;
	...
	protected static void sortExtensions(Map<Integer, byte[]> clientExtensions, Map<Integer, byte[]> copy,
			String order) {
		if (copy == null) {
	...
	/**
	 * Values to account for GREASE (Generate Random Extensions And Sustain
	 * Extensibility) as described here: <a href=
	 * "https://tools.ietf.org/html/draft-davidben-tls-grease-01">draft-davidben-tls-grease-01</a>.
	 */
	private static final int[] GREASE = new int[] { 0x0a0a, 0x1a1a, 0x2a2a, 0x3a3a, 0x4a4a, 0x5a5a, 0x6a6a, 0x7a7a,
